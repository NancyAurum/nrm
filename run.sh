#cc nrm.c -o nrm.exe && ./nrm.exe examples/hello.s out/hello,ff8
#cc nrm.c -o nrm.exe && ./nrm.exe examples/loop.s out/loop,ff8
#cc nrm.c -o nrm.exe && ./nrm.exe examples/get.s out/get,ff8
#cc nrm.c -o nrm.exe && ./nrm.exe examples/macro.s out/macro,ff8
#cc nrm.c -o nrm.exe && ./nrm.exe examples/while.s out/while,ff8
#cc nrm.c -o nrm.exe && ./nrm.exe examples/ifel.s out/ifel,ff8
#cc nrm.c -o nrm.exe && ./nrm.exe examples/pc.s out/pc,ff8
cc nrm.c -o nrm.exe && ./nrm.exe examples/all.s out/all,ff8
